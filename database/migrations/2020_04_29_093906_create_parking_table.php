...
class CreateParkingTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('parking', function (Blueprint $table) {
            ...
            $table->unsignedInteger('size_id')->default(0)->comment('车位大小：10 小车位,20 大车位');
            $table->unsignedInteger('type_id')->default(0)->comment('车位类型：10 认筹,20 竞拍');
            $table->unsignedInteger('parking_recommend')->default(20)->comment('车位推荐10 推荐车位 20 否');
            $table->unsignedInteger('parking_area_id')->default(0)->comment('车位区域id');
            $table->unsignedInteger('parking_floor_id')->default(0)->comment('车位楼层id');
            $table->decimal('price')->default(0.00)->comment('价格');
            $table->decimal('handsel')->default(0.00)->comment('定金');
            $table->unsignedInteger('house_id')->default(0)->comment('关联楼盘id');
